...
testBatch prefix batch index =
    let
        ...
        _ =
            Debug.log "iteration" index
    in
        List.map (\a -> getValue (prefix ++ (toString a))) (List.range start end)
...
main =
    div []
        (List.map (\a -> div [] [ text a ]) (testAllBatches "ffykfhsq" 1000 (List.range 19990 29990)))
